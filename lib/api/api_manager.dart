...
class APIManager {
  ...
  Future response(
    ...
    // CommonController cController = Get.find();
    ...
    try {
      ...
      if (isNeedErrorAlert) {
        ...
        if (e.message.toString().contains("SocketException")) {
          toastPlatform("Check you internet connection!");
        } else {
          if (e.response != null) {
            toastPlatform(e.response!.data["message"]);
          } else {
            toastPlatform("Something went wrong!");
          }
        }
